...
export const Navbar: React.FC<INavbarProps> = ({
  ...
  return (
    <div className="fixed right-0 top-1/2 -translate-y-1/2 z-40 flex items-center justify-end w-20 md:w-24 h-[530px] bg-transparent pr-4 group pointer-events-auto">
      <nav
        className={`flex flex-col items-center gap-2 md:gap-3 bg-background/80 backdrop-blur-md border border-border/40 px-2.5 py-4 rounded-3xl w-fit transition-all duration-300 ease-in-out shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.5)] group-hover:shadow-[0_20px_50px_rgba(0,0,0,0.22)] dark:group-hover:shadow-[0_20px_50px_rgba(0,0,0,0.65)] ${
          isNavbarPinned
        ...
        <TooltipTrigger asChild>
          <Button
            ...
            className={`h-9 w-9 rounded-full cursor-pointer hover:bg-muted ${
              isNavbarPinned ? "text-primary bg-muted" : "text-muted-foreground"
            }`}
      ...
      <div className="flex flex-col items-center bg-muted/60 p-0.5 rounded-2xl gap-1">
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              variant="ghost"
              ...
              className={`h-8 w-8 p-0 rounded-xl cursor-pointer text-xs font-semibold flex items-center justify-center transition-all duration-200 ${
                activeTab === "simple"
